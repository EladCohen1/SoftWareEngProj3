...
public class MainTrain {
	...
	public static void testClient(int port) {
		try {
			...
			Thread reader = new Thread(() -> {
				try {
					PrintWriter out = new PrintWriter(new FileWriter("test/output.txt"), true);
					String line;
			...
			Thread writer = new Thread(() -> {
				try {
					BufferedReader in = new BufferedReader(new FileReader("test/input.txt"));
					String line;
	...
	public static void main(String[] args) {
		...
		server.stop();
		check("test/output.txt", "test/expectedOutput.txt");
